...
Handles all basic preprocessing that runs on every dataset automatically:
- Duplicate removal
- Missing value imputation (median for numerical, mode for categorical)
- Automatic categorical encoding (label for binary, one-hot for multi-class)
- Standard normalization using StandardScaler
- Optional SMOTE for class imbalance (if ratio > 0.2)
- Optional SelectKBest feature selection (if features > 20)
...
def remove_duplicates(X, y, skip_steps):
    ...
    if "duplicates" in skip_steps:
        print("[prepare] Skipping duplicate removal (as configured)")
        return X, y
...
def impute_missing(X, skip_steps):
    ...
    if "imputation" in skip_steps:
        print("[prepare] Skipping missing value imputation (as configured)")
        return X
...
def encode_categoricals(X, y, task_type, skip_steps):
    ...
    if "encoding" in skip_steps:
        print("[prepare] Skipping categorical encoding (as configured)")
        return X, y, {}
    ...
    if len(categorical_cols) == 0:
        ...
        print(f"[prepare] Encoded {len(binary_cols)} binary cols (label encoding), "
...
def normalize(X, skip_steps):
    ...
    print(f"[prepare] Applied StandardScaler to {len(numerical_cols)} numerical features")
    return X, scaler
...
def handle_imbalance(X, y, task_type, skip_steps, threshold=0.2):
    ...
    if "smote" in skip_steps:
        print("[prepare] Skipping SMOTE (as configured)")
        return X, y
    ...
    print(f"[prepare] Class imbalance detected (ratio={imbalance_ratio:.3f}), applying SMOTE")
...
def main(config_path="program.md"):
    ...
    X, scaler = normalize(X, skip_steps)
